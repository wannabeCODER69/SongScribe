...
export default function StudioWorkspace() {
    ...
    const confidenceLabel =
        confidenceValue === null || confidenceValue === undefined || Number.isNaN(Number(confidenceValue))
            ? "-"
    ...
    const artistName = match?.artist || match?.artistName || match?.performer || match?.artist_credit || "";
    ...
    const acoustIdConfidence =
        match?.acoustidConfidence ?? match?.acoustIdConfidence ?? match?.confidence ?? job?.transcript?.confidence;

    const acoustIdConfidenceLabel =
        acoustIdConfidence === null || acoustIdConfidence === undefined || Number.isNaN(Number(acoustIdConfidence))
            ? "-"
                    ...
                    <div className="flex min-h-0 flex-col rounded-[20px] border border-white/9 bg-black/14 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
                        ...
                        <div className="flex min-h-0 flex-1 items-start gap-6">
                            ...
                            <div className="min-w-0 pt-0">
                                ...
                                <p className="mt-2 text-[14px] font-medium text-violet-400">
                                    {artistName || "—"}
                                </p>
                                <p className="mt-2 text-[13px] italic text-white/82">
                                    {albumName || "—"}
                                </p>
                            ...
                            <div className="space-y-3">
                                {transcriptBlocks.length > 0 ? (
                                    ...
                                    <div className="text-white/50">
                                        {job?.processing || job?.uploading ? "Transcription in progress…" : "No transcript yet."}
                                    </div>
                                )}
                            </div>
                        </div>
